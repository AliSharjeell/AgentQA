...
// Update click_target in python to prevent clicking disabled elements
const clickTargetPython = `def click_target():
    active_target = action.get("_target") or target
    if not active_target:
        raise Exception("Target element is required for click action.")
    
    # NEW: Check if the element is known to be disabled
...
code = code.replace(/def click_target\(\):[\s\S]*?click_at_xy\(int\(active_target\.get\("x", 0\)\), int\(active_target\.get\("y", 0\)\)\)/, clickTargetPython);

fs.writeFileSync('src/core/harness.ts', code);
console.log('Fixed harness.ts edge cases');
